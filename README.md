...
**1. Create k8s infrastructure**
- [x] Split microservice infrastructure from base infrastrucutre
- [ ] Allocate resources to the microservice based on env
- [ ] Grant: zero permissions, healtcheck and affinity
- [ ] Ingress control for testing feat branch

**2. IaC with Terraform**
- [x] Manage infrastructure with Terraform
- [x] Lint and check tf file
- [ ] Load provider k8s by environment

**3. Other**
- [x] CI: Create test workflow
- [ ] CD: Create deploy workflow
...
1. The `run.sh` command make it easy to manage the local k8s infrastructure, run these commands in your terminal:
   ...
   ```./localstack/run.sh -t plan```

   ```./localstack/run.sh -t apply --create-localstack``` (opt *--create-localstack* only first run)
2. Expose cluster
